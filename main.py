...
# AUTO-FIX: replace any curly quotes before running

src = open(**file**, “rb”).read()
fixed = src
fixed = fixed.replace(b”\xe2\x80\x98”, b”’”)
fixed = fixed.replace(b”\xe2\x80\x99”, b”’”)
fixed = fixed.replace(b”\xe2\x80\x9c”, b’”’)
fixed = fixed.replace(b”\xe2\x80\x9d”, b’”’)
fixed = fixed.replace(b”\xe2\x80\xa6”, b”…”)
if fixed != src:
open(**file**, “wb”).write(fixed)
os.execv(sys.executable, [sys.executable] + sys.argv)

# –– REAL BOT CODE BELOW ––

import asyncio, json, time, hashlib, aiohttp, feedparser, pandas as pd, ta, websockets
...
“APTUSDT”,“NEARUSDT”,“ATOMUSDT”,“TRXUSDT”,“LTCUSDT”,
“ETCUSDT”,“FILUSDT”,“ICPUSDT”,“HBARUSDT”,“ALGOUSDT”,
“DOTUSDT”,“UNIUSDT”,“AAVEUSDT”,“MKRUSDT”,
]
...
```
    ...
    print("ANALYZE ERR: " + str(e))
    return None
```

def signal_txt(r):
note = “Strong signal” if r[“grade”]==“HIGH” else “Medium signal” if r[“grade”]==“MEDIUM” else “Scalp only”
return “\n”.join([
“BUY [” + r[“grade”] + “] “ + r[“symbol”],
r[“sig”] + “ | Score: “ + str(r[“score”]) + “%”,
note,
“”,
“Entry: $” + fmt(r[“price”]),
“TP1:   $” + fmt(r[“tp1”]) + “ (+” + str(r[“t1p”]) + “%)”,
“TP2:   $” + fmt(r[“tp2”]) + “ (+” + str(r[“t2p”]) + “%)”,
“SL:    $” + fmt(r[“sl”])  + “ (-”  + str(r[“slp”]) + “%)”,
“RR:    1:” + str(r[“rr”]),
“”,
“RSI: “ + str(r[“rsi”]) + “ | TF: 1m+5m | Bybit”,
...
“SL:    $” + fmt(sl)  + “ (-8%)”,
“”,
“Exit 40% TP1 | 40% TP2 | hold 20% TP3”,
“HIGH RISK - new listing!”,
])

def ann_txt(title, link, syms, src):
coins = “ / “.join(syms) if syms else “check link”
return “\n”.join([“EARLY ALERT - Upcoming Listing!”, “Source: “+src, “Coin: “+coins, “”, title, “”, “Prepare before listing!”, link])
...
if cur >= trade[“tp2”]:
await bot.send_message(chat_id=CHAT_ID, text=“TP2 HIT: “+sym.upper()+” $”+fmt(cur)+”\nClose position!”)
del open_trades[sym.upper()]
elif cur >= trade[“tp1”]:
await bot.send_message(chat_id=CHAT_ID, text=“TP1 HIT: “+sym.upper()+” $”+fmt(cur)+”\nMove SL to entry!”)
open_trades[sym.upper()][“sl”] = trade[“price”]
elif cur <= trade[“sl”]:
await bot.send_message(chat_id=CHAT_ID, text=“SL HIT: “+sym.upper()+” $”+fmt(cur)+”\nTrade closed.”)
del open_trades[sym.upper()]
...
except Exception as e: print(“Check listings err: “+str(e))
...
async def cmd_start(u,c):  await u.message.reply_text(“Bot running!\n/status /trades /top /help”)
async def cmd_status(u,c): await u.message.reply_text(“Running\nSymbols: “+str(len(ALL_SYMBOLS))+”\nSignals: “+str(len(last_sig))+”\nTrades: “+str(len(open_trades)))
...
await u.message.reply_text(”\n”.join(lines))
async def cmd_help(u,c): await u.message.reply_text(“Beast Mode | Bybit | 1m+5m\nEMA+MACD+RSI+BB+ATR+Vol\nAuto TP/SL | New listings | Early alerts\nEducational use only.”)
